...
public class PostDAO implements InterfaceDAO<Post> {
	...
	try {
	    String sql = "INSERT INTO Post (username, content) VALUES ('" + post.getUsuario().getNomeUsuario() + "','"
		    + post.getConteudo() + "')";
	    UtilBD.updateDB(sql);

	    sql = "INSERT INTO UserPost (user_fk, post_fk) VALUES (" + post.getUsuario().getId() + "," + getLastId()
		    + ");";
	    UtilBD.updateDB(sql);

	} catch (SQLException e) {
		System.out.println("{ COULDN'T ADD THIS POST }");
	}
    }
	...
	try {
	    String sql = "UPDATE Post SET content = '" + post.getConteudo() + "'" + "WHERE id = " + post.getIdPost()
		    + ";";
	    UtilBD.updateDB(sql);
	} catch (SQLException e) {
		System.out.println("{ COULDN'T SET POST }");
	}
    }
	...
	try {
		...
		System.out.println("{ COULDN'T REMOVE THIS POST }");
	}
    }
	...
	try {
		...
		System.out.println("{ COULDN'T LIST POSTS }");
	}
	return retrn;
    }
	...
	try {
		...
		System.out.println("{ IMPOSSIBLE TO CONSULT POSTS AT DB }");
	}
	return retrn;
    }
